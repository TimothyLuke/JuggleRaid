...
function M:FormatGroupComp(style, t, h, m, r, u, isInRaid)
  if style == M.GROUP_COMP_STYLE.ICONS_FULL then
    return format(
      ...
      M:GetRoleIcon("TANK"),
      h,
      M:GetRoleIcon("HEALER"),
      m + r + u,
      M:GetRoleIcon("DAMAGER"),
  ...
  elseif style == M.GROUP_COMP_STYLE.VERBOSE then
    local unknown =
      (u > 0) and format(", %d %s", u, ((u == 1) and L["word.unknown.singular"] or L["word.unknown.plural"])) or ""
    return format(
...
function M:ColorSystem(text)
  local c =
    format(
    "|cff%02x%02x%02x",
    ChatTypeInfo["SYSTEM"].r * 0xff,
    ChatTypeInfo["SYSTEM"].g * 0xff,
    ChatTypeInfo["SYSTEM"].b * 0xff
  )
